...
interface VisitChecklistPDFData {
  clientName: string
  supervisorName: string
  date: string
  checklistName: string
...
export async function generateVisitChecklistPDF(data: VisitChecklistPDFData): Promise<void> {
  const doc = new jsPDF({
    orientation: 'portrait',
    unit: 'mm',
    format: 'a4',
  ...
  doc.setFontSize(18)
  doc.setFont('helvetica', 'bold')
  doc.text('CHECKLIST DE INSPEÇÃO - PREENCHIDO', margin, yPosition)
  yPosition += 10
  ...
  doc.setFont('helvetica', 'normal')
  doc.setFontSize(11)
  doc.text('Nome do Cliente:', margin, yPosition)
  doc.setFont('helvetica', 'bold')
  doc.text(data.clientName, margin + 45, yPosition)
  doc.setFont('helvetica', 'normal')
  yPosition += 7

  // Supervisor
  doc.text('Nome do Supervisor:', margin, yPosition)
  doc.setFont('helvetica', 'bold')
  doc.text(data.supervisorName, margin + 48, yPosition)
  doc.setFont('helvetica', 'normal')
  yPosition += 7
  ...
  doc.setFont('helvetica', 'bold')
  doc.text(data.date, margin + 20, yPosition)
  doc.setFont('helvetica', 'normal')
  yPosition += 7

  // Setor
  if (data.setor) {
    doc.text('Setor:', margin, yPosition)
    doc.setFont('helvetica', 'bold')
    doc.text(data.setor, margin + 25, yPosition)
    doc.setFont('helvetica', 'normal')
    yPosition += 7
  }
  ...
  // Turno
  if (data.turno) {
    doc.text('Turno:', margin, yPosition)
    doc.setFont('helvetica', 'bold')
    doc.text(data.turno, margin + 25, yPosition)
    doc.setFont('helvetica', 'normal')
    yPosition += 7
  }
  ...
  if (data.nomeColaborador) {
    ...
    doc.setFont('helvetica', 'bold')
    doc.text(data.nomeColaborador, margin + 38, yPosition)
    doc.setFont('helvetica', 'normal')
    yPosition += 7
  }
  yPosition += 5
  ...
  // Locais de Inspeção Preenchidos
  doc.setFont('helvetica', 'bold')
  doc.setFontSize(12)
  doc.text('LOCAIS VISTORIADOS', margin, yPosition)
  yPosition += 8
  ...
  data.locations.forEach((location, index) => {
    ...
    // Checkbox marcado se tiver valor
    doc.setDrawColor(0, 0, 0)
    doc.setLineWidth(0.3)
    doc.rect(margin, yPosition - 4, 4, 4) // Checkbox
    
    if (location.value && location.value.trim() !== '') {
      // Marcar checkbox com X
      doc.setFont('helvetica', 'bold')
      doc.text('X', margin + 1, yPosition - 1)
      doc.setFont('helvetica', 'normal')
    }
    
    doc.text(`${itemNumber}. ${location.name}`, margin + 7, yPosition)
    ...
    if (location.value && location.value.trim() !== '') {
      ...
      doc.setFont('helvetica', 'bold')
      doc.setTextColor(blueDark[0], blueDark[1], blueDark[2])
      doc.text(`Resposta: ${location.value}`, margin + 7, yPosition)
      doc.setTextColor(0, 0, 0)
      doc.setFont('helvetica', 'normal')
      yPosition += 2
    } else {
      // Linha pontilhada se não preenchido
      const lineY = yPosition + 2
      doc.setLineWidth(0.1)
      doc.setDrawColor(200, 200, 200)
      let xPos = margin + 7
  ...
  // Gerar nome do arquivo
  const dateOnly = data.date.split('T')[0] || new Date().toISOString().split('T')[0]
  const fileName = `checklist-visita-${data.clientName.replace(/\s+/g, '-').toLowerCase()}-${dateOnly}.pdf`
